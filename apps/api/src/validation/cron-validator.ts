...
export function validateCron(cron: string): void {
  ...
  if (parts.length !== 5) {
    throw new AtlasError('VALIDATION_ERROR', 'Cron expression must have exactly 5 fields (minute hour day month dow)', 400);
  }
  ...
  if (minute === '*') {
    throw new AtlasError('VALIDATION_ERROR', 'Sub-minute cron schedules are not allowed. Minute field must be specific.', 400);
  }

  if (minute && minute.startsWith('*/') && minute !== '*/1') {
    const step = parseInt(minute.replace('*/', ''), 10);
    if (Number.isNaN(step) || step < 1) {
      throw new AtlasError('VALIDATION_ERROR', 'Invalid cron minute step value', 400);
    }
  }
}
